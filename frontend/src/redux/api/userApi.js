...
export const userApi = createApi({
  ...
  tagTypes: ["User", "Role", "Permission", "ActivityLog"],
  endpoints: (builder) => ({
    // Lấy danh sách người dùng
    getUsers: builder.query({
      query: () => "/users",
    ...
    // Tạo người dùng mới (có thêm token từ localStorage)
    createUser: builder.mutation({
      query: (newUser) => {
        ...
        return {
          ...
          body: newUser,
          headers: {
            Authorization: token ? `Bearer ${token}` : undefined, // Thêm header Authorization nếu có token
          },
        };
      },
