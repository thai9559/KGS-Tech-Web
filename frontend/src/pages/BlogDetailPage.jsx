...
const BlogDetailPage = () => {
    ...
    if (!blog) {
        return <div className="p-4 text-center text-xl text-red-500">Blog không tồn tại.</div>;
    }

    const handleTagClick = (tag) => {
        const tagWithoutHash = tag.replace('#', '').trim();
        setSearchTerm(tagWithoutHash);  // Set search term là tag mà không có '#'
    };
    ...
    return (
        ...
        <div className="max-w-6xl mx-auto p-4 sm:p-6 lg:p-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2">
                <h1 className="text-3xl sm:text-4xl font-bold text-gray-800 mb-6">{blog.title}</h1>

                {/* Thông tin bài viết */}
                <div className="mb-6 text-gray-600">
                    <p><strong className="font-semibold">Ngày đăng:</strong> {blog.date}</p>
                </div>
            ...
            {/* Blog Gợi Ý - Phía bên phải */}
            <div className="lg:col-span-1">
                <SuggestedBlogs blogs={suggestedBlogs} />
                <h2 className='text-black text-2xl font-bold mt-12'>Tag List</h2>
